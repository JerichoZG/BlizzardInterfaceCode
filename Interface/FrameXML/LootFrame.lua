...
function LootFrame_OnLoad(self)
	...
	the atlas, cull the overlaps so the composition appears to fit.]] -- Maps each of the four corners to a UV relative to their corner.
...
function GroupLootFrame_OnShow(self)
	local texture,
		...
		reasonGreed,
		reasonDisenchant,
		deSkillRequired = GetLootRollItemInfo(self.rollID)
	if (name == nil) then
...
function EncounterJournalLinkButtonMixin:OnClick()
	...
	EncounterJournal_OpenJournal(
		difficultyID,
		BonusRollFrame.instanceID,
		BonusRollFrame.encounterID,
		nil,
		nil,
		forceClickLootTab
	)
